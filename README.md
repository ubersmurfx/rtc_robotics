...
cd ~
python3 -m venv venv_rtc_student
```
Клонируем удаленный репозиторий на пульт:
```
git clone -b rev_2.0_С https://github.com/ubersmurfx/rtc_robotics.git
```
Устанавливаем необходимые зависимости для работы:
```
source ~/venv_rtc_student/bin/activate
cd rtc_robotics/
pip install pyyaml
pip install numpy
pip install pynput
...
- o, k – управление третьим звеном манипулятора;
...
# Особенности работы
Важно отметить, что исполняемый файл *onpult.py* (на пульте) должен запускаться только после включения робота. 
